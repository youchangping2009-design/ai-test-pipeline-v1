...
今天的改造重点是让“用例标签、用例生成、开发自测抽取、重跑校验”形成一条更稳定的主链路。

### 1. 标签语义拆分

标签类型不增加，但语义做了拆分：

- `AI-API用例` / `AI-UI用例`：表示适合沉淀的自动化执行载体。
- `开发必测` / `测试必测`：表示人工执行责任。
- 非核心且可由 AI-API / AI-UI 稳定覆盖的用例，不再默认追加人工必测标签。
- 核心流程或关键变更用例，需要同时包含 `开发必测` 与 `测试必测`。
- `开发必测` 优先覆盖接口规则、数据源过滤/排序、指标计算、状态流转、保存前后端校验、低成本可自测的配置落库规则。

落地点：

- `skills/numbering-tagging/rules/tag_rule.yaml`
- `skills/numbering-tagging/SKILL.md`
- `skills/case-generation/SKILL.md`
- `scripts/coverage_testcase_generator.py`
- `skills/case-generation/scripts/testcase_lint.py`

### 2. 开发自测用例派生

新增 `testcases/dev_self_testcases.md`，从主用例真源 `testcases/testcases_main.md` 中筛选标签包含 `开发必测` 的用例生成。

关键约束：

- 不新增 testcase 真源。
- 不新增 JSON 维护成本。
- `dev_self_testcases.md` 是派生产物，主真源仍是 `testcases_main.md`。
- `validate_work_item.py` 会在该文件存在时校验它与主用例中的 `开发必测` 过滤结果一致。

手动重建命令：

```bash
/usr/bin/python3 scripts/build_dev_self_testcases.py \
  --testcases assets/projects/<PROJECT_CODE>/work_items/<WORK_ITEM_ID>/testcases/testcases_main.md \
  --output assets/projects/<PROJECT_CODE>/work_items/<WORK_ITEM_ID>/testcases/dev_self_testcases.md
```

### 3. 用例生成器增强

`coverage_matrix -> testcase` 生成器今天重点增强了以下能力：

- 存在 `case_plan.json` 时，只输出可追溯 CasePlan 的正式候选用例。
- Flow 用例继承真实业务页面和板块，不再兜底到无关页面。
- 中文页面/模块编号使用稳定编码并全局去重。
- `coverage_type=data_rule` 可生成正式数据校验用例，覆盖聚合、均分、指标计算等规则。
- 标题、前置条件、步骤、预期结果改为人类可读表达，机器字段保留在备注追溯中。
- 若需求未明确必填，且原型无 `*` 或等价标识，字段默认按非必填处理，不脑补“为空保存失败”。

相关脚本：

- `scripts/coverage_testcase_generator.py`
- `scripts/generate_testcases_from_coverage.py`
- `scripts/generate_rule_based_testcases_bundle.py`
- `scripts/execute_regeneration_bundle.py`

### 4. 用例元素定义规则

正式 testcase 的步骤和预期结果需要使用统一元素标注，避免“页面、按钮、字段、状态、接口参数”等关键对象在自然语言里失去稳定语义。

当前约定见 `rules/testcase_element_notation.md`，核心口径如下：

- 页面、Tab 使用 `[]`，例如 `[页游落地页管理]`。
- 按钮、入口、操作点使用 `【】`，例如 `【保存】`、`【查询】`。
- 弹窗、抽屉、面板使用 `《》`，例如 `《复制落地页》`。
- 字段、列表列使用中文引号，例如 `“落地页名称”`。
- 枚举值、输入值使用 `{}`，例如 `{已发布}`、`{最近7天}`。
- 状态、结果使用 `<>`，例如 `<保存成功>`、`<未发布>`。
- 提示语 / Toast 使用 `「」`，例如 `「保存成功」`。
- 接口、参数、技术字段使用反引号，例如 `landingPageId`。
...
在 `validate_work_item.py` 中可通过显式参数启用元素标注检查：
